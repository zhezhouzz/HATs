...
let rec type_infer_lit (ctx : Typectx.ctx) (lit : lit) : lit * Nt.t =
  match lit with
  ...
  | ATu l ->
      let l, tys =
        ...
        @@ List.map
             (fun x ->
               let x, ty = type_infer_lit ctx x.x in
               (x #: (Some ty), ty))
             l
      in
      (ATu l, Nt.mk_tuple tys)
  | AProj (a, n) -> (
      let x, aty = type_infer_lit ctx a.x in
      let a = x #: (Some aty) in
      match aty with
  ...
  | AAppOp (f, args) ->
      let args, argsty =
        ...
        @@ List.map
             ...
             args
      in
      let fty = infer_op ctx f.x in
      let f = f.x #: (Some fty) in
      let argsty, retty = _solve_by_argsty __FILE__ __LINE__ fty argsty in
      let args =
        List.map (type_check_lit ctx)
          (_safe_combine __FILE__ __LINE__ args argsty)
      in
      (AAppOp (f, args), retty)

and type_check_lit (ctx : Typectx.ctx) (lit, ty) : lit typed =
  (* let () = *)
  ...
  match lit with
  | AC _ | AVar _ ->
      let x, ty' = type_infer_lit ctx lit in
  ...
  | ATu l ->
      ...
      let l =
        List.map
          (fun (x, ty) -> type_check_lit ctx (x.x, ty))
          (_safe_combine __FILE__ __LINE__ l tys)
      in
      { x = ATu l; ty = Some (Nt.Ty_tuple tys) }
  | AProj (_, _) ->
      let lit, ty' = type_infer_lit ctx lit in
      let ty = _check_equality __FILE__ __LINE__ Nt.eq ty ty' in
      { x = lit; ty = Some ty }
  | AAppOp (f, args) ->
      let args, argsty =
        List.split @@ List.map (fun x -> type_infer_lit ctx x.x) args
      in
      let fty = infer_op ctx f.x in
      let f = f.x #: (Some fty) in
...
let type_check_qualifier (ctx : Typectx.ctx) (qualifier : prop) : prop =
  let ty = Nt.bool_ty in
  let rec aux ctx qualifier =
    match qualifier with
    | Lit lit -> Lit (type_check_lit ctx (lit, ty)).x
    ...
    | Forall (u, body) ->
        let ctx' = Typectx.new_to_right ctx u in
