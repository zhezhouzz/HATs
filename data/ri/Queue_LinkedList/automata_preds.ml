...
let[@pred] nextP (prev : Cell.t) (value : Cell.t) =
  _F
    ...
    && _X
         (_G
            (not
               (DelNext ((prev [@d]), x_1, v, true)
               || SetNext ((prev [@d]), x_1, v, true)))))

let[@pred] has_nextP (prev : Cell.t) =
  _F
    (SetNext ((prev [@d]), x_1, v, true)
    && _X (_G (not (DelNext ((prev [@d]), x_1, v, true)))))

let[@pred] has_prevP (a : Cell.t) =
  _F
    (SetNext (x_0, (a [@d]), v, true)
    && _X (_G (not (DelNext (x_0, (a [@d]), v, true)))))
