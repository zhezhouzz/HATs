type effect =
  | Reset of (unit -> unit)
  | Hasnext of (unit -> bool)
  | Next of (unit -> unit)
  | Getcur of (unit -> int)
  | Insertaftercur of (int -> unit)
...
let add_edge (vtab : nat) (in_tab : nat) (out_tab : nat) (eid : int)
    (start_id : int) (end_id : int) : unit =
...
let[@assert] add_edge ?l:(vtab = (true : [%v: nat]))
    ...
    ?l:(eid = (true : [%v: int])) ?l:(start_id = (true : [%v: int]))
